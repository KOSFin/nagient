...
Bundled-плагины — это совместимый fallback, а не второй источник установки. Они
лежат в `nagient/bundled_*` внутри wheel, поэтому первый запуск работает offline.
Внешние репозитории имеют отдельные ID и устанавливаются в runtime home. Сейчас
Telegram и GitHub существуют в обеих формах намеренно: bundled-ID сохраняют старые
конфигурации, а проверенные репозитории позволяют выпускать обновления отдельно.
Когда внешняя замена станет стабильной, bundled-копию можно убрать в следующем
major-релизе через миграцию, не ломая существующие профили.
...
Транспорт с progressive delivery может объявить namespaced stream-функцию
(`telegram.streamMessage` для Telegram). Runtime передаёт накопленные текстовые
снимки, а транспорт отвечает за редактирование сообщения, лимиты длины и batching
по rate limit. Так streaming провайдера не зависит от API конкретного транспорта.
