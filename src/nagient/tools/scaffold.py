...
def _render_readme(plugin_id: str) -> str:
    return "\n".join(
        [
            ...
            "",
            "Python plugins export `build_plugin()` from `tool.py`.",
            "Any-language plugins may set `runtime = \"process\"` in `tool.toml` and expose",
            "the same functions through the Nagient JSON process protocol on stdin/stdout.",
            "",
