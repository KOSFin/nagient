# Контекст для AI и автоматизации

[English](README.md) · Русский · [Документация разработчика](../docs/developer/README.ru.md) · [Обзор проекта](../README.ru.md)
...
| Раздел | Назначение |
| --- | --- |
| [Назначение проекта](#1-назначение-проекта) | Продуктовые и delivery-цели. |
| [Текущее состояние](#2-текущее-состояние) | Реализованные возможности runtime. |
| [Карта репозитория](#3-карта-репозитория) | Основные модули и границы расширений. |
| [Стабильные контракты](#4-инварианты-которые-нельзя-ломать) | Сквозные инварианты. |
| [CLI-поверхность](#5-текущая-cli-поверхность) | Поддерживаемые группы команд. |
| [Фокус тестирования](#6-фокус-тестирования) | Обязательное regression coverage. |
| [Рабочий промпт](#8-рабочий-промпт) | Подробный implementation context. |
...
Runtime уже включает выполнение provider, tool calls, approvals, jobs, session memory, transports, health reporting и управляемый lifecycle. Подробный implementation prompt считается историческим контекстом там, где он расходится с текущим кодом.
